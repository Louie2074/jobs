...
Dumps cap_tap_full.json + cap_tap_net.json (artifacts) and a log-readable summary to stdout.
"""
...
async def _tp_state(tab, stage):
    ...
    try:
        print(f"[TP {stage}] " + str(await tab.evaluate(js))[:700], flush=True)
    except Exception as e:
        print(f"[TP {stage}] err {str(e)[:60]}", flush=True)


async def drive_tap(tab):
    """Discovery drive of the LifeMiles award search (flytap.com/fly/find): dismiss consent,
    ...
    for w in range(8):
        ...
        print(f"[TP WAIT {(w+1)*5}s] {cnt}", flush=True)
        try:
            import json as _j
    ...
    print(f"[TP PROBE] {str(probe)[:900]}", flush=True)
...
async def main():
    ...
    port = free_port()
    profile = tempfile.mkdtemp(prefix="tpcap_")
    ...
    try:
        ...
        try:
            ...
            if isinstance(html, str):
                with open("cap_tap_results.html", "w") as f:
        ...
        try:
            ...
            if isinstance(cards_html, str):
                with open("cap_tap_cards.json", "w") as f:
                    f.write(cards_html)
    ...
    with open("cap_tap_full.json", "w") as f:
        json.dump(cap, f)
    analyze(cap)

    # CDP Network-layer capture (catches service-worker / SSR responses)
    with open("cap_tap_net.json", "w") as f:
