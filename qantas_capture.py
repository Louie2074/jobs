...
Dumps cap_qantas_full.json + cap_qantas_net.json (artifacts) and a log-readable summary to stdout.
"""
...
WARM_URL = "https://www.qantas.com/us/en/book-a-trip/flights.html"
ORIGIN_CITY, ORIGIN_CODE = "Los Angeles", "LAX"
DEST_CITY, DEST_CODE = "Sydney", "SYD"
...
async def _qf_state(tab, stage):
    ...
    try:
        print(f"[QF {stage}] " + str(await tab.evaluate(js))[:700], flush=True)
    except Exception as e:
        print(f"[QF {stage}] err {str(e)[:60]}", flush=True)


async def drive_qantas(tab):
    """Discovery drive of the LifeMiles award search (lifemiles.com/fly/find): dismiss consent,
    ...
    for w in range(8):
        ...
        print(f"[QF WAIT {(w+1)*5}s] {cnt}", flush=True)
        try:
            import json as _j
    ...
    print(f"[QF PROBE] {str(probe)[:900]}", flush=True)
...
async def main():
    ...
    port = free_port()
    profile = tempfile.mkdtemp(prefix="qfcap_")
    ...
    try:
        ...
        try:
            ...
            if isinstance(html, str):
                with open("cap_qantas_results.html", "w") as f:
        ...
        try:
            ...
            if isinstance(cards_html, str):
                with open("cap_qantas_cards.json", "w") as f:
                    f.write(cards_html)
    ...
    with open("cap_qantas_full.json", "w") as f:
        json.dump(cap, f)
    analyze(cap)

    # CDP Network-layer capture (catches service-worker / SSR responses)
    with open("cap_qantas_net.json", "w") as f:
