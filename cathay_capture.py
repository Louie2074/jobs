...
Dumps cap_cathay_full.json + cap_cathay_net.json (artifacts) and a log-readable summary to stdout.
"""
...
WARM_URL = "https://www.cathaypacific.com/cx/en_US/book-a-trip/redeem-flights/redeem-flight-awards.html"
ORIGIN_CITY, ORIGIN_CODE = "New York", "JFK"
DEST_CITY, DEST_CODE = "Hong Kong", "HKG"
...
async def main():
    ...
    port = free_port()
    profile = tempfile.mkdtemp(prefix="cathaycap_")
    ...
    try:
        ...
        try:
            ...
            if isinstance(html, str):
                with open("cap_cathay_results.html", "w") as f:
        ...
        try:
            ...
            if isinstance(cards_html, str):
                with open("cap_cathay_cards.json", "w") as f:
                    f.write(cards_html)
    ...
    with open("cap_cathay_full.json", "w") as f:
        json.dump(cap, f)
    analyze(cap)

    # CDP Network-layer capture (catches service-worker / SSR responses)
    with open("cap_cathay_net.json", "w") as f:
